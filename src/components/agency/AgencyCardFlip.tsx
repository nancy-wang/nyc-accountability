...
const TREND_ARROW: Record<TrendDirection, { glyph: string; color: string } | null> = {
  improving: { glyph: "▲", color: "var(--trend-improving)" },
  worsening: { glyph: "▼", color: "var(--trend-worsening)" },
  flat: null,
  ...
  return (
    <div className="[perspective:1200px]" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      ...
      <div
        ...
        className="relative h-[36rem] w-[19rem] cursor-pointer text-left [transform-style:preserve-3d] transition-transform duration-500 motion-reduce:transition-none"
      ...
      >
        ...
        >
          <div className="relative flex items-center justify-center px-5 py-2" style={{ background: "var(--brand-blue)" }}>
            <span
              aria-hidden
              className="absolute left-2 top-1/2 -translate-y-1/2 -rotate-6 rounded-full border px-1.5 py-0.5 text-[7px] font-bold tracking-wide"
          ...
          <div className="flex flex-1 flex-col items-center justify-center gap-2 p-3">
            <img src={sealPath} alt="" aria-hidden className="h-20 w-20 object-contain" />
          </div>

          <div className="flex flex-col items-center gap-0.5 px-3 py-2" style={{ background: "var(--brand-red)" }}>
            <span className="max-w-full text-center text-[9px] font-semibold uppercase leading-tight tracking-wide" style={{ color: "var(--brand-cream)" }}>
              {topicTitle}
            </span>
            <span className="text-[10px] font-bold" style={{ color: "var(--brand-cream)" }}>
              Tap to flip ↻
            </span>
          </div>
        </div>
        ...
        <div
          ...
          style={{ borderColor: "var(--brand-blue)", background: "var(--brand-cream)", boxShadow: shadow, clipPath: CARD_CLIP }}
        >
          <div className="flex items-center justify-center px-5 py-1.5" style={{ background: "var(--brand-blue)" }}>
            <h3 className="font-display text-center text-xs leading-tight" style={{ color: "var(--brand-gold)", textShadow: TITLE_SHADOW }}>
              {agencyName}
            </h3>
          </div>

          <div className="flex flex-1 flex-col overflow-y-auto p-2.5">
            <ul className="space-y-1 text-[10.5px] leading-snug" style={{ color: "var(--text-secondary)" }}>
            ...
            <AgencyTrendBar trend={trend} compact />

            {topIndicators.length > 0 && years.length > 0 && (
              <div className="mt-2">
                <div
                  className="grid items-start gap-x-1 gap-y-1.5 text-[9px] leading-tight"
                  style={{ gridTemplateColumns: `minmax(0,1fr) repeat(${years.length}, minmax(1.85rem, auto))` }}
                >
                  <span />
                  {years.map((fy) => (
                    <span key={fy} className="text-right font-semibold" style={{ color: "var(--text-muted)" }}>
                      {formatFiscalYear(fy)}
                    </span>
                  ))}
                    ...
                    return (
                      <Fragment key={indicator.id}>
                        <span className="flex min-w-0 items-start gap-0.5">
                          ...
                          <span style={{ color: "var(--text-secondary)" }}>{indicator.name}</span>
                          {arrow && (
                            <span aria-hidden className="shrink-0 text-[8px]" style={{ color: arrow.color }} title={indicator.trend}>
                              {arrow.glyph}
                            </span>
                          )}
                        </span>
                        {years.map((fy) => {
                          ...
                          return (
                            <span key={fy} className="text-right tabular-nums" style={{ color: "var(--text-primary)" }}>
                ...
                {topIndicators.some((indicator) => indicator.years.some((y) => y.isProjected)) && (
                  <p className="mt-0.5 text-right text-[8px]" style={{ color: "var(--text-muted)" }}>
                    *projected, partial-year
                  </p>
                )}
              </div>
            )}
            ...
            <div className="mt-2">
              ...
              <p className="mt-0.5 text-[10px] leading-snug" style={{ color: "var(--text-secondary)" }}>
                {standoutSummary}
              </p>
            </div>
          </div>

          <div className="flex items-center justify-center px-3 py-1.5" style={{ background: "var(--brand-red)" }}>
            <Link
